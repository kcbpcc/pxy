...
def calculate_timpxy():
    ...
    start_time_utc = datetime.datetime.utcnow().replace(hour=3, minute=01, second=0, microsecond=0, tzinfo=pytz.utc)
    ...
    if start_time_utc <= current_datetime_utc <= end_time_utc:
        ...
        timpxy = max(5, round(15 - (total_minutes / total_time_range_minutes) * 10, 2))
        return timpxy
    else:
