...
from ordcnsloptpxy import cancel_existing_orders
...
output_lines = [
    "{:<30}".format(f"BNK ━━━━━━> {BRIGHT_GREEN if bmktpredict == 'RISE' else BRIGHT_RED if bmktpredict == 'FALL' else BRIGHT_YELLOW}{bmktpredict} {arrow_map.get(bmktpxy, '')}{RESET}") +
    "{:>30}".format(f"{BRIGHT_GREEN if mktpredict == 'RISE' else BRIGHT_RED if mktpredict == 'FALL' else BRIGHT_YELLOW}{arrow_map.get(nmktpxy, '')} {mktpredict}{RESET} <━━━━━━ NFT")
]
print('\n'.join(output_lines))
...
lmt_ord_df = exe_opt_df[exe_opt_df['quantity'] > 0].copy()
