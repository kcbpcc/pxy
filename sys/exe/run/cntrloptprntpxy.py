...
output_lines.append(left_aligned_format.format(f"Run-oPnL:{BRIGHT_GREEN if run_opnl > 0 else BRIGHT_RED}{str(round(run_opnl/100000, 2)).zfill(4)}{RESET}") +
                    right_aligned_format.format(f"Cash: {BRIGHT_GREEN if live_balance > 50000 else BRIGHT_YELLOW}{str(round(live_balance / 100000, 2)).zfill(6)}{RESET}"))
...
output_lines.append(left_aligned_format.format(
        f"Stocks-dPnL:{BRIGHT_GREEN if all_Stocks_worth_dpnl > 0 else BRIGHT_RED}{int(round(all_Stocks_worth_dpnl, 0))}{RESET}") +
        right_aligned_format.format(
        f"Stocks-bPnL:{GREEN if booked > 0 else RED}{str(int(booked)).zfill(5)}{RESET}"))
