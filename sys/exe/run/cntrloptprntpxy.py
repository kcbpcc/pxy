...
output_lines.append(left_aligned_format.format(
        ...
        right_aligned_format.format(
        f"Cash-Left:{BRIGHT_GREEN if live_balance > 50000 else BRIGHT_YELLOW}{int(round(live_balance, 0)):04d}{RESET}"))
#output_lines.append(
...
for group, data in grouped_df:
    ...
    if total_invested_group != 0:
        ...
        value_statement = f"  {pe_count:02d} -🟥- {valtotal_invested_peue_pe:06d}  {Istrike}   {total_invested_ce:06d}  -🟩- {ce_count:02d}"
...
summary = (
    f"---------PXY® Dash Board----------\n"
    f"          C&C-tCap  :   {CnC_tCap_rounded:<8}\n"
    f"          C&C-tPnL  :   {round(run_spnl / 100000, 2):<8}\n"
    f"          F&O-tCap  :   {round(total_invested_all / 100000, 2):<8}\n"
    ...
    f"          BAK-bPnL  :   {str(bank_profit).zfill(5):<8}\n"
    f"          NFT-bPnL  :   {str(nifty_profit).zfill(5):<8}\n"
