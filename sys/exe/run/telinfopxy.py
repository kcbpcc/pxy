...
async def send_telegram_message(message_text):
    ...
    await bot.send_message(chat_id=user_usernames, text=message_text, parse_mode=telegram.ParseMode.MARKDOWN_V2)
...
async def run_and_send_message():
    try:
        ...
        telegram_message = f"Program Output:\n\n`{output}`"  # Using backticks for inline code formatting
        await send_telegram_message(telegram_message)

    except subprocess.CalledProcessError as e:
        # Handle errors if the subprocess fails
        error_message = f"Error running the program: `{e}`"  # Using backticks for inline code formatting
