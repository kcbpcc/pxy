...
def printbord(Day_Change, result, total_PnL_percentage, total_dPnL, total_PnL, total_dPnL_percentage,
    ...
    output_lines.append(left_aligned_format.format(f"{BOLD}{UNDERLINE}My Options{RESET}") +
                        right_aligned_format.format(f"{BOLD}{UNDERLINE}My Stocks{RESET}"))
    output_lines.append(
        ...
        right_aligned_format.format(
            ...
            f"{BOLD}{UNDERLINE}®{RESET}       S-Count:{BRIGHT_YELLOW}{str(all_Stocks_count).zfill(3)}{RESET}"
        )
    )    
