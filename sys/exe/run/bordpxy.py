...
def printbord(Day_Change, result, total_PnL_percentage, total_dPnL, total_PnL, total_dPnL_percentage,
    ...
    #output_lines.append(left_aligned_format.format(f"NSE Day%:{BRIGHT_GREEN if Day_Change >= 0 else BRIGHT_RED}{round(Day_Change, 2)}{RESET}") +
                        #right_aligned_format.format(f"NSE Open%:{BRIGHT_GREEN if Open_Change >= 0 else BRIGHT_RED}{round(Open_Change, 2)}{RESET}")) 
    ...
    output_lines.append(
        left_aligned_format.format(
            ...
            f"         {BOLD}{UNDERLINE}PX{RESET}{BRIGHT_GREEN if mktpxy in ['Bull', 'Buy'] else BRIGHT_RED}"
        ) +
        right_aligned_format.format(
            f"{BRIGHT_GREEN if mktpxy in ['Bull', 'Buy'] else BRIGHT_RED}"
            f"{BOLD}{UNDERLINE}Y®{RESET}        Gainers:{BRIGHT_GREEN}{str(green_Stocks_count).zfill(3)}{RESET}"
        )
    )    
