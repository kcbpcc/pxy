...
def printbord(total_cnc_m2m_postions, extras, optworth, all_Stocks_worth_dpnl, nsma, all_Stocks_yworth_lacks, total_opt_m2m, mktpxy, available_cash, ha_nse_action, nse_power, Day_Change, Open_Change, all_Stocks_count, red_Stocks_count, green_Stocks_count, all_Stocks_capital_lacks, all_Stocks_worth_lacks, zero_qty_count, green_Stocks_profit_loss, green_Stocks_capital_percentage):
    ...
    output_lines.append(left_aligned_format.format(f"B-mktpxy:{BRIGHT_GREEN if green_Stocks_capital_percentage > 0 else BRIGHT_RED}{bmktpxy}{RESET}") +
                        right_aligned_format.format(f"N-mktpxy:{BRIGHT_GREEN if green_Stocks_capital_percentage > 0 else BRIGHT_RED}{mktpxy}{RESET}"))                                          
    output_lines.append(
    ...
    output_lines.append(left_aligned_format.format(f"Flush:{BRIGHT_GREEN if green_Stocks_profit_loss > 0 else BRIGHT_RED}{round(green_Stocks_profit_loss)}{RESET}") +
                        right_aligned_format.format(f"Day-pP&L:{BRIGHT_GREEN if total_cnc_m2m_postions > 0 else BRIGHT_RED}{str(int(total_cnc_m2m_postions))}{RESET}"))
    output_lines.append(left_aligned_format.format(f"Day-oP&L:{BRIGHT_GREEN if total_opt_m2m > 0 else BRIGHT_RED}{int(round(total_opt_m2m, 0))}{RESET}") +
                        right_aligned_format.format(f"Day-hP&L:{BRIGHT_GREEN if all_Stocks_worth_dpnl > 0 else BRIGHT_RED}{int(round(all_Stocks_worth_dpnl, 0))}{RESET}"))
