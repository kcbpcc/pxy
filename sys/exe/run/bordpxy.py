...
def printbord(Day_Change, result, total_PnL_percentage, total_dPnL, total_PnL, total_dPnL_percentage,
    ...
    output_lines.append(left_aligned_format.format(f"A/C Cap:{BRIGHT_YELLOW}{round(capital, 2)}{RESET}") +
                        right_aligned_format.format(f"A/C Now:{BRIGHT_YELLOW}{round(acvalue_to_print + 3.58, 2)}{RESET}"))
    ...
    output_lines.append(
        ...
        right_aligned_format.format(
            ...
            f"{BOLD}{UNDERLINE}®{RESET}          {GREY}{'My Stocks'.zfill(3)}{RESET}"
        )
    )
    ...
    output_lines.append(left_aligned_format.format(f"Cap:{BRIGHT_YELLOW}{round((nrmlall_Stocks_capital/100000), 2)}{RESET}") + 
    ...
    output_lines.append(left_aligned_format.format(f"tUnreal:{BRIGHT_GREEN if total_PnL_nrml_buy >= 0 else BRIGHT_RED}{int(total_PnL_nrml_buy)}{RESET}") +
                        right_aligned_format.format(f"tUnreal:{BRIGHT_GREEN if total_PnL >= 0 else BRIGHT_RED}{round((total_PnL/100000), 2)}{RESET}"))
    output_lines.append(left_aligned_format.format(f"dReal:{BRIGHT_GREEN if result_nrml >= 0 else BRIGHT_RED}{str(int(result_nrml)).zfill(5)}{RESET}") +
                        right_aligned_format.format(f"dReal:{BRIGHT_GREEN if result > 0 else BRIGHT_RED}{str(round(result)).zfill(5)}{RESET}"))
    output_lines.append(left_aligned_format.format(f"Drive:{BRIGHT_GREEN if auto_value_status == 'Yes' else BRIGHT_RED}{auto_value}{RESET}") +
                        right_aligned_format.format(f"dProfit:{BRIGHT_GREEN if (result_nrml+result) > 0 else BRIGHT_RED}{int(round((result_nrml+result), 2))}{RESET}"))   
