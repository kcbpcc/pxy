async def place_order(broker, symbol, transaction_type, product_type, quantity, order_type, price=None):
    def extract_strike(symbol):
        return int(symbol[-7:-2])  # Extract the last 7 digits minus the last 2 (CE or PE)
    ...
    def check_existing_positions(broker, symbol):
        ...
        for position in positions_response['net']:
    ...
    if not success:
        ...
        for alt_symbol in alternate_symbols:
            # Check if the alternate symbol is already in position
            if not check_existing_positions(broker, alt_symbol):
