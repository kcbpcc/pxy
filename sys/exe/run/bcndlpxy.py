...
def get_niftyfifty_data(period="7d"):
...
def get_today_close():
    niftyfifty_ohlc = get_niftyfifty_data(period="1d")
    if not niftyfifty_ohlc.empty:
        prev_close = get_previous_day_close(niftyfifty_ohlc)
        return niftyfifty_ohlc.iloc[-1]['Close'], prev_close
    else:
        return None, None
...
def option_to_trade():
    today_data = get_niftyfifty_data().iloc[-1][OHLC_COLUMNS]
    today_close = today_data['Close']
    option_value = round(today_close / 50) * 50
    return option_value

previous_day_close = get_previous_day_close(get_niftyfifty_data())
today_close = get_today_close()

if previous_day_close is not None and today_close is not None:
    niftyfifty_ohlc = get_niftyfifty_data(period="1d")
    today_data = niftyfifty_ohlc.iloc[-1][OHLC_COLUMNS]
...
if previous_day_close is not None and today_close is not None:
    close = today_close[0]
    open_price = niftyfifty_ohlc.iloc[-1]['Open']
