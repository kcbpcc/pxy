...
try:
    ...
    threshold = 3
###########################################################################################################################################################################################################
    combined_df['fPL%'] = combined_df['smb_power'].apply(lambda x: round(np.exp(np.clip((smb_power), -threshold, threshold)), 2))
    combined_df['tPL%'] = combined_df['fPL%'].apply(lambda x: max(round(np.exp(np.clip((nse_power), -threshold, threshold)), 2), 1.4))
