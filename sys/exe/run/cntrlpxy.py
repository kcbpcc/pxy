...
###########################################################################################################################################################################################################
    if not nrml_filtered_df.empty:
        width = 42 
        df_output = nrml_filtered_df[['key', 'qty', 'otPL%', 'PL%', 'PnL']].to_string(index=False, header=False)
        print(f'{auto_value}'.rjust(width) + '\n' + '\n'.join(line.rjust(width) for line in df_output.split('\n')))
