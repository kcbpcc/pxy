...
###########################################################################################################################################################################################################
    ...
    PRINT_df = pxy_df[['source','product','key', 'tPL%','PL%','PnL','qty','smb_power']]
...
###########################################################################################################################################################################################################    
    ...
    PRINT_df_sorted['key'] = PRINT_df_sorted['key'].str.replace(r'(BSE:|NSE:|NFO:)', '', regex=True).str[:11].str.ljust(11, ' ')
    # Sort the DataFrame by 'PL%' in ascending order
    ...
    stocks_filtered_df = PRINT_df_sorted_display[(PRINT_df_sorted_display['PL%'] > 1.4 ) & (PRINT_df_sorted_display['Q'] == '+') & (PRINT_df_sorted_display['_CM'] == '🧰')]
