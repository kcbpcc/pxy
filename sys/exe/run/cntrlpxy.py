...
###########################################################################################################################################################################################################
    ...
    combined_df[['pr', 'xl', 'yi', '_pr', '_xl', '_yi']] = combined_df.apply(
        lambda row: pd.Series({
            ...
            '_pr': round(min(-0.4, round(0.0 + (row['weakness'] * 1.0), 2) * 1 - epsilon), 2),
            '_xl': round(min(-0.8, round(0.0 + (row['weakness'] * 1.0), 2) * 1.5 - epsilon), 2),
            '_yi': round(min(-1.2, round(0.0 + (row['weakness'] * 1.0), 2) * 2 - epsilon), 2),
        }), axis=1
    )
...
###########################################################################################################################################################################################################
    ...
    PRINT_df = pxy_df[['source','product','key','pxy','dPL%','oPL%','PL%','PnL','qty','smbchk']]
...
###########################################################################################################################################################################################################    
    ...
    filtered_df = PRINT_df_sorted_display[PRINT_df_sorted_display['PnL'] > ({TIMPXY}/2)]
