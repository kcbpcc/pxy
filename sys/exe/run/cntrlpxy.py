...
try:
    ...
    combined_df['hsl'] = combined_df.apply(lambda row: (row['ltp'] - row['open']) / (row['open'] - row['ostp']) if (row['ltp'] - row['open']) != 0 else 1, axis=1)
...
###########################################################################################################################################################################################################    
    ...
    numeric_columns = ['smbchk','hsl','pstp','_pstp','qty', 'average_price', 'Invested','Yvalue', 'ltp','close', 'open', 'high', 'low','value', 'PnL', 'PnL%','PnL%_H', 'dPnL', 'dPnL%']
...
###########################################################################################################################################################################################################
    ...
    pxy_df = filtered_df.copy()[['smbchk','hsl','pstp','_pstp','source','product', 'qty','average_price', 'close', 'ltp', 'open', 'high','low','pxy','yxp','key','dPnL%','PnL','PnL%_H', 'PnL%']]
    ...
    PRINT_df = PRINT_df.rename(columns={'source': 'X', 'product': 'Y', 'qty' : 'Q','smbchk': 'I'})
    ...
    PRINT_df['I'] = PRINT_df['I'].apply(lambda I: '🟢' if I == 'Bull' else ('🔴' if I == 'Bear' else ('🌚' if I == 'Sell' else ('🌞' if I == 'Buy' else I))))
    ...
    PRINT_df_sorted['I'] = PRINT_df_sorted['I'].apply(lambda I: I[:2] if isinstance(I, str) else I)
