...
###########################################################################################################################################################################################################
    ...
    combined_df['otPL%'] = combined_df['smb_power'].apply(lambda x: round(np.exp(np.clip(((x ) / 1), -threshold, threshold)), 2))
...
####################1#######################################################################################################################################################################################                    
                        ...
                        auto_value == 'AUTO' and
                        (('CE' in row['key'] and row['PL%'] > 140 and mktpxy in ["Sell", "Bear"]) or
                         ('PE' in row['key'] and row['PL%'] > 140 and mktpxy in ["Buy", "Bull"]))
                    ):
                        try:                            
