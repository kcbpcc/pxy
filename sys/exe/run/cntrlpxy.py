            ...
            if order_id:
                ...
                with open(csv_file_path, 'a', newline='') as csvfile:
                    ...
                    try:
                        import telegram
                        import asyncio
                        columns_to_drop = ['fPL%','tPL%','oPL%','smb_power', 'oPL%', 'pstp', '_pstp', 'qty', 'close', 'open', 'high', 'low', 'PL%_H', 'dPL%', 'pxy','yxp']
...
###########################################################################################################################################################################################################    
    ...
    PRINT_df_sorted['key'] = PRINT_df_sorted['key'].str.replace(r'(BSE:|NSE:|NFO:)', '', regex=True).str[:14].str.ljust(14, ' ')
    # Sort the DataFrame by 'PL%' in ascending order
...
###########################################################################################################################################################################################################                    
                    if (
                        (row['qty'] > 0 and
                         ...
                         row['PL%'] > 1.4 ) and
                        (
                            ((row['PL%'] > row['tPL%']) and (row['PL%'] > 200)) or ((row['PL%'] > 0) and (total_dPnL < 0))
                        )
                    ):
                        try:
