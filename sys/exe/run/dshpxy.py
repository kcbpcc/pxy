...
def get_holdingsinfo(combined_df):
    try:
        ...
        total_opt_m2m_positions = cnc_nfom2m_df['m2m'].sum()
        ...
        all_Stocks_capital_lakhs = all_Stocks_capital / 100000
        all_Stocks_worth_lakhs = all_Stocks_worth / 100000
        all_Stocks_yworth_lakhs = all_Stocks_yworth / 100000
        green_Stocks_capital_lakhs = green_Stocks_capital / 100000
        red_Stocks_capital_lakhs = red_Stocks_capital / 100000

        selected_opts_df = combined_df[(combined_df['qty'] != 0) & combined_df['key'].str.contains('NFO:', case=False)].copy()
        total_opts_invested_lakhs = selected_opts_df['Invested'].sum() / 100000

        sum_CE = combined_df[(combined_df['key'].str.contains('NFO:')) & (combined_df['qty'] > 0) & (combined_df['key'].str.endswith('CE'))]['Invested'].sum()
        sum_PE = combined_df[(combined_df['key'].str.contains('NFO:')) & (combined_df['qty'] > 0) & (combined_df['key'].str.endswith('PE'))]['Invested'].sum()
        ...
        return (
            total_opt_m2m_positions, nifty_extras, optworth, all_Stocks_worth_dpnl, 
            all_Stocks_yworth_lakhs, nifty_total_opt_m2m, all_Stocks_count, red_Stocks_count, 
            green_Stocks_count, all_Stocks_capital_lakhs, all_Stocks_worth_lakhs, zero_qty_count, 
            green_Stocks_profit_loss, green_Stocks_capital_percentage
        )
