...
def get_holdingsinfo(csv_file_path):
    try:
        ...
        table = PrettyTable()
        table.field_names = ['🧘', '🟢🔴🟢', '🟩🟩🟩', '🟥🟥🟥','💰💰💰']
        table.add_row(['📈', total_Stocks_count, green_Stocks_count, red_Stocks_count,0000])
        table.add_row(['💰', convert_to_laks(all_Stocks_capital), convert_to_laks(green_Stocks_capital), convert_to_laks(red_Stocks_capital), 0000])
        table.add_row(['🔄', convert_to_laks(all_Stocks_worth), convert_to_laks(green_Stocks_worth), convert_to_laks(red_Stocks_worth),0000])

        if all_Stocks_profit_loss < 0:
            table.add_row(['💵', f'{Style.BRIGHT}{Fore.RED}{format_value(all_Stocks_profit_loss)}{Style.RESET_ALL}', colorize(green_Stocks_profit_loss), colorize(red_Stocks_profit_loss)])
        else:
            table.add_row(['💵', f'{format_value(all_Stocks_profit_loss)}', colorize(green_Stocks_profit_loss), colorize(red_Stocks_profit_loss)])
