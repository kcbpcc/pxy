...
def send_telegram_message(bot_token, user_usernames, message):
    try:
...
def exit_options(exe_opt_df, broker, bot_token, user_usernames):
    ...
    try:
        for index, row in exe_opt_df.iterrows():
            ...
            if total_pl_percentage > tgtoptsmadepth and row['PnL'] > 400:
                ...
                print(message)
                send_telegram_message(bot_token, user_usernames, message)
    except Exception as e:
