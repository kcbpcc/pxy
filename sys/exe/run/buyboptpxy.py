...
def check_existing_positions(broker, symbol):
    ...
    positions_net = positions_response['net']
    for position in positions_net:
        if position['tradingsymbol'] == symbol and abs(position['quantity']) >= 15:
            return True
    return False
...
async def main():
    ...
    try:
        ...
        print(f"{BRIGHT_YELLOW}📉{count_PE:02}:PE positions💧 {showhand} 🔥CE positions:{count_CE:02}📈{RESET}")
        ...
        await process_orders(broker, available_cash, CE_position_exists, PE_position_exists, CE_symbol, PE_symbol, count_CE, count_PE)
