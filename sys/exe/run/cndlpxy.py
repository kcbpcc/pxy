...
print(f"🔆{GREEN if Day_Change >= 0 else RED}{day_change_sign}{int(Day_Change):.2f}⌛️{GREEN if open_change_sign >= 0 else RED}{open_change_sign}{int(Open_Change):.2f}⚡{GREEN if nse_power >= 0 else RED}{int(nse_power):.2f} {nsmawave} 🟥-{pedepth}🚦{macd}🚦🟩+{cedepth}")
