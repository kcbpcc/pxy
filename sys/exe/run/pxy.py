while True:
    ...
    def get_market_check(symbol):
        ...
        if current_color == 'Bear' and last_closed_color == 'Bear':
            ...
            subprocess.run(['python3', 'sellpxy.py']) if nse_action in ['DangerBear'] else None
            subprocess.run(['python3', 'cntrlpxy.py'])
        elif current_color == 'Bull' and last_closed_color == 'Bull':
            mktpxy = 'Bull'
            console.print("🐂🟢🟢🟢 [bold]Bullish sentiment![/bold] 💪💰", style=bull_style)
            subprocess.run(['python3', 'buypxy.py']) if nse_action in ['SuperBull', 'SuperBear','DangerBull'] else None
            subprocess.run(['python3', 'cntrlpxy.py'])
