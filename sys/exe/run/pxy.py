...
class Tee:
    ...
    def write(self, message):
        ...
        self.stdout.write(message)  # Write to the terminal

    def flush(self):
        self.file.flush()  # Ensure that the file is flushed
        self.stdout.flush()  # Ensure that the terminal is flushed

def main():
    with open('screen.rtf', 'w') as file:
        sys.stdout = Tee(file)  # Create a Tee instance and set it as sys.stdout
        ...
        while True:
            ...
            subprocess.run(['python3', 'finalpxy.py'])
            ############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################    ############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################ 
            ...
            ############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################    ############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################ 
        # Reset standard output back to terminal
